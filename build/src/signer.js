...
class EIP712Signer {
    ...
    static getSignInput(transaction) {
        ...
        const signInput = {
            ...
            paymaster: transaction.customData?.paymasterParams?.paymaster || ethers_1.ethers.constants.AddressZero,
...
class Signer extends (0, adapters_1.AdapterL2)(ethers_1.ethers.providers.JsonRpcSigner) {
    ...
    async sendTransaction(transaction) {
        ...
        if (transaction.customData == null && transaction.type != utils_1.EIP712_TX_TYPE) {
            ...
            transaction.gasPrice ?? (transaction.gasPrice = await this.provider.getGasPrice());
            transaction.gasLimit ?? (transaction.gasLimit = await this.provider.estimateGas(transaction));
            transaction.chainId ?? (transaction.chainId = (await this.provider.getNetwork()).chainId);
